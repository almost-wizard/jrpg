...
public class GhostComponent extends EnemyComponent {
    private static final int WIDTH = 16;
    private static final int HEIGHT = 40;
    private final CharacterAnimation ghostAnimation = new GhostAnimation();
    private AiComponent aiComponent;

    public GhostComponent(EnemyImportance importance, Rectangle area, Point2D startPosition) {
    ...
    public void onAdded() {
        ...
        animation.loopAnimation(ghostAnimation.getIdle());
        entity.addComponent(animation);

        entity.setLocalAnchor(ghostAnimation.getSpriteSize().multiply(0.5));
        ...
        inventory.getModel().resetItems(Remnant.getGhostRemnants());
        entity.addComponent(inventory);

        EntityState idle = new IdleState(entity, ghostAnimation);
        AiAttack aiAttack = new AiAttack(entity, ghostAnimation, false, false, area, startPosition);
        AttackState attack = new AttackState(entity, ghostAnimation, aiAttack);
        ReturningState returning = new ReturningState(entity, ghostAnimation, attack.getAiAttack());
        EntityState deadMinable = new DeadMinableState(entity, ghostAnimation, getImportance());
    ...
    private Attack[] getAttacks() {
        var bbox = BoundingShape.box(ghostAnimation.getSpriteWidth() / 2.0, ghostAnimation.getSpriteHeight());

        return new Attack[]{
                new Attack(AttackOwner.ENEMY, bbox, 0, Duration.seconds(0.5), ghostAnimation.getAttack())
                        .setWeapon(new AiNonPhysical(30, 50, 1.5)),
        };
    }
